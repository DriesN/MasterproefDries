function cost = costfunction(x,inputs,methode)    
    ...
    if strcmp(methode, 'systeemidentificatie_1zone_metUFH')
        ...
        for i = 1:length(T_meas)-1
            T_cal(i+1) = T_cal(i) + (Q_solar_meas(i)+Q_heat_meas(i)-C_v.*((T_floor(i+1)-T_floor(i))./(t(i+1)-t(i)))-(T_cal(i)-T_amb_meas(i))./R)./C .*(t(i+1)-t(i));
            Q_heat_meas(i) = ((T_floor(i)-T_cal(i))./R_v)+C_v.*(T_floor(i+1)-T_floor(i))./(t(i+1)-t(i));
        end
    end
