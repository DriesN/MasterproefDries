...
%optimalisatie tweede methode (met vloerverwarming)
if strcmp(methode,'systeemidentificatie_1zone_metUFH')
    ...
    for i = 1:length(inp.T_meas)-1        
        ...
        T_floor(i+1) = T_floor(i) + ((Q_heat(i)-((T_floor(i)-T_cal(i))./R_v))./C_v).*(inp.t(i+1)-inp.t(i));
    end
    for i = 1:3
        T_cal = rot90(T_cal);
    end
...
%optimalisatie derde methode (met interne winsten en vloerverwarming--> splitsing oppervlakte en kern)
